...
loadProjects(page: number = this.currentPage) {
    const pagination = { page: page, page_size: this.pageSize };
    this.apiService.getProjects(pagination).subscribe({
        next: (data) => {
            this.projects = data.content; // Assuming 'content' holds the array of projects
            this.totalProjects = data.meta.total; // Assuming 'meta.total' holds the total count
